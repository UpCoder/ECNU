...
def gaze(frame, points):
    ...
    if transformation is not None:  # if estimateAffine3D secsseded
        ...
        # print(pupil_world_cord[0], pupil_world_cord[1])
        ...
        eye_dist = ((image_points[2][0] - image_points[3][0])**2 + (image_points[2][1] - image_points[3][1])**2)**0.5
        dist = ((p2[0]-p1[0])**2 + (p2[1]-p1[1])**2)**0.5

        h, v = 0, 0
        if dist > eye_dist/3:
            if p2[0]-p1[0] > eye_dist/3:
                h = 1
            elif p2[0]-p1[0] < -eye_dist/3:
                h = -1

            if p2[1]-p1[1] > eye_dist/3:
                v = -1
            elif p2[1]-p1[1] < -eye_dist/3:
                v = 1
        # font = cv2.FONT_HERSHEY_SIMPLEX
        # cv2.putText(frame, "{}:{}".format("H", h), (20, 20),
        #             font, 0.6, (0, 0, 0), 1, cv2.LINE_AA)
        # cv2.putText(frame, "{}:{}".format("V", v), (20, 40),
        #             font, 0.6, (0, 0, 0), 1, cv2.LINE_AA)
        gaze_infos = {
            "gaze_x":S[0][0],
            "gaze_y":S[1][0],
            "gaze_z":S[2][0],
            "gaze_horizontal_orientation":h,
            "gaze_vertical_orientation":v
        }
        return gaze_infos