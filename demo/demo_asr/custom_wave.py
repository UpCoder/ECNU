...
def write_binary(rate, data):
    ...
    * The bits-per-sample and PCM/float will be determined by the data-type.
    ...
    try:
        ...
        binary_result += header_data
        # fid.write(header_data)

        # data chunk
        # fid.write(b'data')
        # fid.write(struct.pack('<I', data.nbytes))
        binary_result += b'data'
        binary_result += struct.pack('<I', data.nbytes)

        if data.dtype.byteorder == '>' or (data.dtype.byteorder == '=' and
                                           sys.byteorder == 'big'):
            data = data.byteswap()
        # _array_tofile(fid, data)
        binary_result += data.ravel().view('b').data
        ...
        # fid.seek(4)
        # fid.write(struct.pack('<I', size-8))
        size = len(binary_result)
        binary_result = binary_result[:4] + struct.pack('<I', size-8) + binary_result[4:]
