...
app_id = 28232561
app_key = 'KFqpIX29d0o6iQA1SL5nc7ZH'
...
if len(sys.argv) < 2:
    ...
    pcm_file = sys.argv[1]
pcm_base_name = pcm_file.split('.')[0]
...
def on_close(ws):
    ...
    :return:
    """
    logger.info(f"ws close ..., {len(final_msgs)}")
    # ws.close()
    json.dump({
        'msgs': final_msgs
    }, open(f'{pcm_base_name}.json', 'w'), ensure_ascii=False)
    for final_msg in final_msgs:
        logging.info(final_msg)
    for final_msg in final_msgs:
        logging.info(final_msg['result'])
